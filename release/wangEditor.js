...
UploadVideo.prototype = {
    ...
    uploadVideo: function uploadVideo(files) {
        ...
        if (!customUploadVideo && !uploadVideoServer) {
            return;
        }
            ...
            xhr.onreadystatechange = function () {
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status < 200 || xhr.status >= 300) {
                        ...
                        // xhr 返回状态错误
                        _this2._alert('上传视频发生错误', '\u4E0A\u4F20\u89C6\u9891\u53D1\u751F\u9519\u8BEF\uFF0C\u670D\u52A1\u5668\u8FD4\u56DE\u72B6\u6001\u662F ' + xhr.status);
                        return;
                    }
                    ...
                    if (!hooks.customInsert && result.msgCode != '0') {
                        // hook - fail
                        if (hooks.fail && typeof hooks.fail === 'function') {
                            hooks.fail(xhr, editor, result);
                        }
                        ...
                        // 数据错误
                        _this2._alert('上传视频失败', '上传视频返回结果错误，返回结果 msgCode=' + result.msgCode);
                    } else {
                        if (hooks.customInsert && typeof hooks.customInsert === 'function') {
                            // 使用者自定义插入方法
                            ...
                            // 将图片插入编辑器
                            _this2.insertLinkVideo(result.datas.ossUrl);
                        }
